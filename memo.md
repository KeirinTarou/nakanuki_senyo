...
## アプリの仕様
### UI
- 画像プレビュー領域（キャンバス）: `NakanukiApp.canvas`
- 現在の画像高さを表示するラベル: `NakanukiApp.lbl_height`
- 中抜き領域指定スピンボックス
    - 「From」: `NakanukiApp.spin_from`
        - バインドしたコマンド: `NakanukiApp.update_lines()`
    - 「To」: `NakanukiApp.spin_to`
        - バインドしたコマンド: `NakanukiApp.update_lines()`
- 省略線追加用チェックボックス: `NakanukiApp.chk_add_break_line`
- 「画像を開く」ボタン
    - バインドしたコマンド: `NakanukiApp.load_image()`
- 「中抜適用」ボタン
    - バインドしたコマンド: `NakanukiApp.apply_nakanuki()`
- 「中抜き！」ボタン
    - バインドしたコマンド: `NakanukiApp.nakanuki_and_save()`
- 「終了」ボタン
    - - バインドしたコマンド: `tkinter.Tk.quit()`
### 基本的な使い方
- ｢画像を開く」ボタンをクリック
- ファイルダイアログで画像ファイルを選択
    - キャンバスに画像が表示される
...
- スピンボックスで中抜き領域を指定
    - 指定した高さの位置に水平線を表示
    - 水平線の位置はスピンボックスの値に連動する
        - `0`の場合は表示しない
        - 画像高さを超える値のときは表示しない
- 「中抜適用」ボタン or 「中抜き！」ボタンクリック
    - 「中抜適用」ボタンクリック時
        - 指定領域を取り除き、上下を詰めた画像をキャンバス領域に表示する
    - 「中抜き！」ボタンクリック時
        - 指定領域を取り除き、上下を詰めた画像を元画像のフォルダにエクスポートする
            - 保存ファイル名は元のファイル名にタイムスタンプを追加したもの
            - 保存時にファイル名の指定はできない
            - From、Toともに`0`のときは、表示している画像をそのままエクスポートする
                - 「省略線追加」チェックボックスがオンでも省略線は追加しない
- 「省略線追加」チェックボックスがオンのとき
    - 中抜き画像のつなぎ目のところに、`breakline.png`（透過png）を高さが40px、幅が画像幅になるようにリサイズして貼り付ける
    - 中抜き画像のつなぎ目と`breakline.png`の垂直方向の中心を合わせる
    - つなぎ目よりも上、つなぎ目よりも下が20px未満の場合、省略線が見切れることになる
...
- `test_nakanuki_app_calc_horizontal_line_coords.py`
    - 水平線の座標計算ロジックを保証するテスト
        - 幅・高さともにキャンバスよりも大きい
        - 幅・高さともにキャンバスよりも小さい
        - 幅がキャンバスよりも大きい
        - 高さがキャンバスよりも大きい
- `test_nakanuki_image_regression.py`
...
- `test_nakanuki_image_content.py`
