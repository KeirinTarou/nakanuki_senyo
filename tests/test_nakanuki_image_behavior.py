...
def test_nakanuki_image_force_rgb_save(tmp_path):
    img = Image.open("test_assets/BA-90.png")
    result = nakanuki_image(img, 320, 520)
...
def test_nakanuki_image_mode_and_convert(tmp_path):
    img = Image.open("test_assets/BA-90.png")
    result = nakanuki_image(img, 320, 520)
    # 強制RGB化
    result = result.convert("RGB")
...
def test_nakanuki_image_strip_icc_fix(tmp_path):
    img = Image.open("test_assets/BA-90.png")
    result = nakanuki_image(img, 320, 520)

    flat = result.convert("RGB")
    out = tmp_path / "out.png"
    flat.save(out)
    reloaded = Image.open(out)
